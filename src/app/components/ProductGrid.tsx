...
export default function ProductGrid({
	...
	const filteredProducts = useMemo(() => {
		// retrun array kosong jika products undefiend
	...
	return (
		<div className="space-y-8">
			...
			<div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-white p-4 rounded-2xl shadow-sm focus:ring-2 focus:ring-indigo-100">
			...
			<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
				{filteredProducts.map((p: any) => {
					...
					return (
						<Link
							href={`/products/${p.id}`} // Navigasi ke ID
							key={p.id}
			...
			{filteredProducts.length === 0 && (
				<div className="p-20 text-center bg-white rounded-[2rem] border border-dashed border-gray-200">
					<p className="text-gray-400 font-bold italic">Produk not found</p>
				</div>
			)}
		</div>
	);
